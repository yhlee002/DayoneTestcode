...
public class MyCalculatorRepeatableTest {
  ...
  public void parameterizedComplicatedCalculateTest(
      Double addVal, Double minusVal, Double multiplyVal, Double divideVal, Double expectVal) {
    // given
    MyCalculator cal = new MyCalculator(0.0);

    // when
    Double result =
        cal.add(addVal).minus(minusVal).multiply(multiplyVal).divide(divideVal).getResult();
